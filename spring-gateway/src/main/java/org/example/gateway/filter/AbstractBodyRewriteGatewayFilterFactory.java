...
public abstract class AbstractBodyRewriteGatewayFilterFactory<C> extends AbstractGatewayFilterFactory<C> {
    ...
    protected Mono<Void> rewrite(ServerWebExchange exchange, GatewayFilterChain chain,
        ...
        Mono<byte[]> mono = modifiedRequestBody(exchange, requestTrans);

        BodyInserter<Mono<byte[]>, ReactiveHttpOutputMessage> inserter = BodyInserters.fromPublisher(mono, byte[].class);

        Mono<Void> insert = inserter.insert(outputMessage, new BodyInserterContext());
    ...
    private Mono<byte[]> modifiedRequestBody(ServerWebExchange exchange, RewriteFunction<byte[], byte[]> requestTrans) {
        return ServerRequest.create(exchange, messageReaders).bodyToMono(byte[].class) // TODO invoked getBody() method
    ...
    protected class ReactiveCachedBodyOutputMessage implements ReactiveHttpOutputMessage {
        ...
        public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
            // this.body = Flux.from(body).flatMap(dataBuffer -> {
            //     DataBuffer retainedSliceBuffer = dataBuffer.retainedSlice(0, dataBuffer.readableByteCount());
            //     return Mono.just(retainedSliceBuffer)
            //             .doOnDiscard(PooledDataBuffer.class, db -> {
