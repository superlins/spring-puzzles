 ...
 */
public class SimpleSftpInboundFileSynchronizer
    ...
    @Nullable
    private Comparator<ChannelSftp.LsEntry> comparator;
     ...
     */
    public SimpleSftpInboundFileSynchronizer(SessionFactory<ChannelSftp.LsEntry> sessionFactory) {
        Assert.notNull(sessionFactory, "sessionFactory must not be null");
        this.remoteFileTemplate = new RemoteFileTemplate<ChannelSftp.LsEntry>(sessionFactory);
        this.sessionFactory = sessionFactory;
        doSetFilter(new SftpPersistentAcceptOnceFileListFilter(new SimpleMetadataStore(), "sftpMessageSource"));
    }

    public SessionFactory<ChannelSftp.LsEntry> getSessionFactory() {
        return sessionFactory;
    }

    @Nullable
    protected Comparator<ChannelSftp.LsEntry> getComparator() {
     ...
     * @since 5.1
     */
    public void setComparator(@Nullable Comparator<ChannelSftp.LsEntry> comparator) {
     ...
     */
    public void setFilter(@Nullable FileListFilter<ChannelSftp.LsEntry> filter) {
        doSetFilter(filter);
    }

    protected final void doSetFilter(@Nullable FileListFilter<ChannelSftp.LsEntry> filterToSet) {
    ...
    private void walkDirectory(Session<ChannelSftp.LsEntry> session, String remoteDir, Map<String, ChannelSftp.LsEntry> collector) throws IOException {
        ChannelSftp.LsEntry[] files = session.list(remoteDir);
    ...
    private Integer transferFilesFromRemoteToLocal(String remoteDirectory, File localDirectory,
                                                   int maxFetchSize, Session<ChannelSftp.LsEntry> session) throws IOException {

        if (session.exists(remoteDirectory)) {

            // 递归当前远程目录下所有文件
            Map<String, ChannelSftp.LsEntry> entries = new HashMap<>();
            ...
            if (!ObjectUtils.isEmpty(entries)) {
                ...
                for (Map.Entry<String, ChannelSftp.LsEntry> entry : entries.entrySet()) {
                    ...
                    copied = copyIfNotNull(remoteDirectory, localDirectory, localFileEvaluationContext, session, entries, copied, entry);
                }
    ...
    private int copyIfNotNull(String remoteDirectory, File localDirectory,
                              @Nullable EvaluationContext localFileEvaluationContext, Session<ChannelSftp.LsEntry> session,
                              Map<String, ChannelSftp.LsEntry> entries, int copied, @Nullable Map.Entry<String, ChannelSftp.LsEntry> entry) throws IOException {

        boolean renamedFailed = false;
        try {
            if (entry != null && !copyFileToLocalDirectory(remoteDirectory, localFileEvaluationContext, entry, localDirectory, session)) {
                renamedFailed = true;
            }
    ...
    protected boolean copyFileToLocalDirectory(String remoteDirectoryPath,
                                               ...
                                               Map.Entry<String, ChannelSftp.LsEntry> remoteEntry,
                                               File localDirectory,
                                               Session<ChannelSftp.LsEntry> session) throws IOException {

        String remoteFilePath = remoteEntry.getKey();

        // 根据远程文件目录计算最后的本地路径
        Matcher matcher = Pattern.compile("(?<=(\\d{8})).*").matcher(remoteFilePath);
        String realFilePath = matcher.group(1) + matcher.group(0);
        String localFileName = generateLocalFileName(realFilePath, localFileEvaluationContext);

        if (!isFile(remoteEntry.getValue())) {
            if (this.logger.isDebugEnabled()) {
    ...
    private void resetFilterIfNecessary(ChannelSftp.LsEntry remoteFile) {
        if (this.filter instanceof ResettableFileListFilter) {
            ...
            ((ResettableFileListFilter<ChannelSftp.LsEntry>) this.filter).remove(remoteFile);
        }
    }

    private boolean copyRemoteContentToLocalFile(Session<ChannelSftp.LsEntry> session, String remoteFilePath, File localFile) {
        boolean renamed;
    ...
    protected boolean isFile(ChannelSftp.LsEntry file) {
